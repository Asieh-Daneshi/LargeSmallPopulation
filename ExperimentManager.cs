...
public class ExperimentManager : MonoBehaviour
{
	...
	private IEnumerator GeneratePositions(int numARC, int numAngle, int blockIndex1)						   			    // in this coroutine, we generate an array of positions that each avatar will be located (myPos)
    {
		...
		for (int i1 = 1; i1 <= numARC; i1++) 
		{
			...
			{
				xPos.Add((i1*1.2f+5f+Random.Range(-0.002f,0.002f))*Mathf.Sin(((2*j1-1)*(6f+Random.Range(-0.005f,0.005f)))*(Mathf.PI)/180f));
				zPos.Add(-((i1*1.2f+5f+Random.Range(-0.002f,0.002f))*Mathf.Cos(((2*j1-1)*(6f+Random.Range(-0.005f,0.005f)))*(Mathf.PI)/180f)-2));
				ct++;
				xPos.Add(-(i1*1.2f+5f+Random.Range(-0.002f,0.002f))*Mathf.Sin(((2*j1-1)*(6f+Random.Range(-0.005f,0.005f)))*(Mathf.PI)/180f));
				zPos.Add(-((i1*1.2f+5f+Random.Range(-0.002f,0.002f))*Mathf.Cos(((2*j1-1)*(6f+Random.Range(-0.005f,0.005f)))*(Mathf.PI)/180f)-2));
				ct++;
