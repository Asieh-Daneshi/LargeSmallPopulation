...
public class ExperimentManagerGlove : MonoBehaviour
{
	...
	public GameObject ParticipantHead;
    public List<GameObject> MaleMultitude;
    public List<GameObject> FemaleMultitude;
	...
	public List<Animator> MaleMultitudeAnimator;
    public List<Animator> FemaleMultitudeAnimator;
    ...
    void Start()
    {
        ...
        for (int a = 0; a < 45; a++)
        {
			FemaleMultitudeAnimator[a] = FemaleMultitude[a].GetComponent<Animator>();
			pickAnimMultitude = Random.Range(1, 10);         // pick a random integer between 1 and 9 for female multitude
            FemaleMultitudeAnimator[a].SetInteger("Move", pickAnimMultitude);
            //FemaleMultitudeAnimator[a].SetActive(true);
        }
		pickAnimMultitude = 0;
        for (int a = 0; a < 75; a++)
        {
            MaleMultitudeAnimator[a] = MaleMultitude[a].GetComponent<Animator>();
            pickAnimMultitude = Random.Range(1, 10);         // pick a random integer between 1 and 9 for male multitude
			print(a + "      ;        " + pickAnimMultitude);
            MaleMultitudeAnimator[a].SetInteger("MoveMan", pickAnimMultitude);
            //FemaleMultitudeAnimator[a].SetActive(true);
        }
    }
    ...
    {	
	 ...
	 //   }
		...
		while(ParticipantHead.transform.position.y <= 0.5f)
		{
			// print(ParticipantHead.transform.position.x);
			print(ParticipantHead.transform.position.y);
			yield return new WaitForSeconds(Time.deltaTime);
			// 
		}
		pAnimator.SetInteger("Turn", 1);
		
		
		
		responses = new float[400,12];		// The array that we are going to store the parameters of the experiment and participants' responses in it
		...
		for (int ip = 0; ip < numberOfPracticeTrials; ip++) // practice session
		{
			...
			responses[trialCounter, 10] = referenceTime;
			...
			while (!((Listner.lUp || Listner.rUp) || (Time.time - referenceTime) >= trialDuration))
			{
				yield return new WaitForSeconds(Time.deltaTime);
			}
			...
			responses[trialCounter, 2] = randomThreshold; // indicates the dominant color in the stimulus
			responses[trialCounter, 3] = numberOfAgents; // number of responding agents
			responses[trialCounter, 4] = LR; // right hand:2; left hand:1
			responses[trialCounter, 6] = Time.time - referenceTime; // response time
			responses[trialCounter, 7] = congruencyFactor;
			responses[trialCounter, 8] = Time.time - originalTime;
			raisedGloveColor = responses[trialCounter, 5];
			BaseString = 0;
			for (int a = 0; a < numberOfAgents; a++)
			{
				BaseString = int.Parse(BaseString + respondingAgents[a].ToString());
			}
			responses[trialCounter, 9] = BaseString;
			...
			yield return new WaitForSeconds(trialDuration-(Time.time-referenceTime));
			//print("trialDuration: "+trialDuration+" , "+Time.time+ " , " + referenceTime);
	...
	public void WriteCSV(float[,] CSVresponses)
	{
		...
		tw = new StreamWriter("OutputData.CSV", true);
		//print("Asi.CSV");

		for (int TC = 1; TC <= (numberOfPracticeTrials+numberOfTestTrials*2); TC++)
		{
			// responses[trialCounter,0]=SessionInd; 	// if index==1, it is practice session, else, it is test session
			// responses[trialCounter,1]=catchInd; 	// it is 1 if it is a catch. Otherwise, it is zero
			// responses[trialCounter,2]=threshold; 	// yellow:0.6; blue:0.4
			// responses[trialCounter,3]=numberOfAgents; 			// number of responding agents
			// responses[trialCounter,4]: agents' raise hand: right:2; left:1
			// responses[trialCounter,5]: participant's raised hand. right(blue):1; left(yellow):2
			// responses[trialCounter,6]: participant's response time
			// responses[trialCounter,7]=congruencyFactor: congruent:1; incongruent:2
			tw.WriteLine(CSVresponses[TC, 0] + "," + CSVresponses[TC,1] + "," + CSVresponses[TC,2] + ","+ CSVresponses[TC, 3] + "," + CSVresponses[TC, 4]+ "," + CSVresponses[TC, 5]+ "," + CSVresponses[TC, 6]+ "," + CSVresponses[TC, 7]+ "," + CSVresponses[TC, 8]+ "," + CSVresponses[TC, 9]+ "," + CSVresponses[TC, 10]);
